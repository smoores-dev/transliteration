...
const toRawRomaji = function (
  ...
  if (
    system === ROMANIZATION_SYSTEM.NIPPON ||
    system === ROMANIZATION_SYSTEM.HEPBURN
  ) {
    const reg_hatu = new RegExp(
      ...
      (r =
        romajiSystem[system][
          str.substring(
            pnt,
            pnt + 2
          ) as keyof (typeof romajiSystem)[typeof system]
        ])
    ) {
      result += r;
      pnt += 2;
  ...
  if (
    system === ROMANIZATION_SYSTEM.PASSPORT ||
    system === ROMANIZATION_SYSTEM.HEPBURN
  ) {
  ...
  if (system === ROMANIZATION_SYSTEM.HEPBURN) {
    result = result.replace(/aー/gm, 'a');
    result = result.replace(/iー/gm, 'i');
    result = result.replace(/uー/gm, 'u');
    result = result.replace(/eー/gm, 'e');
    result = result.replace(/oー/gm, 'o');
  }
 ...
 */
const patchTokens = function (tokens: Token[]) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    if (
      tokens[i].pos &&
      tokens[i].pos === '助動詞' &&
      (tokens[i].surface_form === 'う' || tokens[i].surface_form === 'ウ')
    ) {
